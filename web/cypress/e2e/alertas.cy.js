describe('Validações da Alertas em JavaScript', ()=> {
    beforeEach(()=>{
        cy.login()
    ...
    it('Deve cancelar um diálogo e validar a resposta positiva', ()=>{
           cy.on('window:confirm', (msg) =>{
            expect(msg).to.equal('Aperte um botão!')
    ...
    it('Deve interagir com um prompt, inserir um texto e validar uma mensagem', ()=>{
        cy.window().then((win) => {
            cy.stub(win, 'prompt').returns('Sueli')
        })

        cy.on('window:alert', (msg)=> {
            expect(msg).to.equal('Olá Sueli! Boas-vindas ao WebDojo!')
        })
